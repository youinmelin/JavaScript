...
<a href="ende_input.php">返回</a> &emsp;
...
if($_POST) {
    ...
    if ($_POST["type"] == "encrypt") {
        ...
        // echo "原文key_ascii".$key_ascii."<br>";
        // echo "密文是：";
        $message = strtoascii($message); // 原文ascii 
        echo "原文ascii：".$message."(".asciitostr($message).$message.")<br>";
        $message_code = encrypt($message, $key); // 加密后ascii
        echo "加密后ascii: ".$message_code."<br>";
        $message_ascii_verity = decrypt($message_code,$key); // 解密后ascii
        echo "解密后ascii: ".$message_ascii_verity."<br>"; // 这有问题!
        $message_verify = asciitostr($message_ascii_verity);
        echo "解密后原文：".$message_verify."<br>";
        if ($_POST["message"] == $message_verify){
            ...
            echo "加密失败，请重试。<br>";
            echo "密文是：<input type='text' id='code' readonly value='". $message_code. "' ><br>";
            echo "<br><input type='button' onclick='copy();' value='点击复制密文'></input><br>";
            echo "密文是：". $message_code. "<br>";
        }
